...
class PLCClient:
    ...
    def read_digital_inputs(self):
        """Leer 14 entradas digitales"""
        try:
            if not self.connected:
                return []
            
            # Leer bytes de entradas digitales
            data = self.client.db_read(1, 0, 2)  # DB1, offset 0, 2 bytes
            inputs = []
            
            for i in range(14):  # 14 entradas digitales
                byte_index = i // 8
                bit_index = i % 8
    ...
    def read_digital_outputs(self):
        """Leer 10 salidas digitales"""
        try:
            if not self.connected:
                return []
            
            # Leer bytes de salidas digitales
            data = self.client.db_read(1, 2, 2)  # DB1, offset 2, 2 bytes
            outputs = []
            
            for i in range(10):  # 10 salidas digitales
                byte_index = (i + 16) // 8  # Ajustar offset
                bit_index = (i + 16) % 8
                value = get_bool(data, byte_index, bit_index)
                outputs.append({
    ...
    def read_analog_inputs(self):
        """Leer 2 entradas analógicas"""
        try:
            if not self.connected:
                return []
            
            # Leer entradas analógicas
            data = self.client.db_read(1, 4, 4)  # DB1, offset 4, 4 bytes
            inputs = []
            
            for i in range(2):
                value = get_int(data, i * 2)  # 2 bytes por valor
                inputs.append({
                    "id": f"AI{i}",
                    "value": value,
...
def plc_real_data():
    ...
    try:
        ...
        digital_inputs = plc.read_digital_inputs()
        digital_outputs = plc.read_digital_outputs()
        analog_inputs = plc.read_analog_inputs()
